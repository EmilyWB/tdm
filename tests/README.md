...
2. **Run tests:**
   ```bash
   docker run --rm -v "$(pwd)/..:/workspace" -w /workspace/tests tdm-scheduler-tests bash -c "
     mkdir -p build &&
     cd build &&
     cmake .. &&
     make &&
...
```bash
docker run -it --rm -v "$(pwd)/..:/workspace" -w /workspace/tests/build tdm-scheduler-tests bash
